                        ...
                        <div class="card-content">
                            ...
                            <div class="content">
                                ...
                                @if (!$solicitud->cuestionario)
                                    <span class="tag is-light">Cuestionario pendiente</span>
                                @endif

                                @if ($solicitud->motivo_rechazo)
                                    <div class="mt-2">
                                        <p class="has-text-danger is-size-7"><strong>Motivo:</strong> {{ $solicitud->motivo_rechazo }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        <footer class="card-footer">
                            <a href="{{ route('mascotas.public.show', $solicitud->mascota) }}" class="card-footer-item">Ver mascota</a>
                            @if (!$solicitud->cuestionario && $solicitud->status === 'pendiente')
                                <a href="{{ route('solicitudes.cuestionario', $solicitud) }}" class="card-footer-item has-text-primary">Completar cuestionario</a>
                            @endif
                        </footer>
                    </div>
                </div>
