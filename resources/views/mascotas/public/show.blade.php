...
@section('content')
    <nav class="breadcrumb" aria-label="breadcrumbs">
        <ul>
            <li><a href="{{ route('mascotas.public.index') }}"><span class="icon is-small"><i class="fas fa-paw"></i></span> Mascotas</a></li>
    ...
    <div class="columns">
        <div class="column is-half">
            <figure class="image">
                ...
                @else
                    <img src="/img/default_mascota.png" alt="Sin foto" style="border-radius: 8px;">
                @endif
            </figure>
        </div>
        ...
        <div class="column is-half">
            ...
            <div class="box">
                <h3 class="subtitle is-5"><span class="icon is-small"><i class="fas fa-align-left"></i></span> Descripción</h3>
                <p>{{ $mascota->descripcion }}</p>
            </div>

            <div class="box">
                <h3 class="subtitle is-5"><span class="icon is-small"><i class="fas fa-building"></i></span> Refugio</h3>
                ...
                @if ($mascota->shelter->phone)
                    <p><span class="icon is-small"><i class="fas fa-phone"></i></span> {{ $mascota->shelter->phone }}</p>
                @endif
            ...
            @if ($mascota->vacunas->isNotEmpty() || $mascota->eventosMedicos->isNotEmpty())
                <div class="box">
                    <h3 class="subtitle is-5"><span class="icon is-small"><i class="fas fa-notes-medical"></i></span> Historial médico</h3>

                    @if ($mascota->vacunas->isNotEmpty())
                        <h4 class="subtitle is-6 mt-3"><span class="icon is-small"><i class="fas fa-syringe"></i></span> Vacunas</h4>
                        <table class="table is-fullwidth is-narrow">
                            <thead>
                                <tr>
                                    <th>Vacuna</th>
                    ...
                    @if ($mascota->eventosMedicos->isNotEmpty())
                        <h4 class="subtitle is-6 mt-3"><span class="icon is-small"><i class="fas fa-heartbeat"></i></span> Eventos médicos</h4>
                        <table class="table is-fullwidth is-narrow">
                            <thead>
                                <tr>
                                    <th>Fecha</th>
            ...
            @auth
                    ...
                    <div class="buttons">
                        <form action="{{ route('solicitudes.create', $mascota) }}" method="POST" class="mr-2">
                            ...
                            <button type="submit" class="button is-primary is-medium" @if($mascota->status !== 'disponible') disabled @endif>
                                @if($mascota->status === 'disponible')
                                    <span class="icon is-small"><i class="fas fa-paper-plane"></i></span> Quiero adoptar
                                @else
                                    <span class="icon is-small"><i class="fas fa-ban"></i></span> No disponible
                                @endif
                            </button>
                        </form>
                        ...
                        <form action="{{ route('favoritos.toggle', $mascota) }}" method="POST">
                            ...
                            <button type="submit" class="button is-info is-light is-medium">
                                ...
                                @if ($esFavorito)
                                    <span class="icon is-small"><i class="fas fa-heart-broken"></i></span> Quitar de favoritos
                                @else
                                    <span class="icon is-small"><i class="fas fa-heart"></i></span> Guardar en favoritos
                                @endif
                            </button>
                        </form>
                    </div>
                @elseif (Auth::user()->role === 'refugio')
                    <div class="notification is-info is-light">
                        ...
                        <a href="{{ route('refugio.mascotas.index') }}"><span class="icon is-small"><i class="fas fa-paw"></i></span> Ir a mis mascotas</a>
                    </div>
                @endif
            @else
                <div class="notification is-info is-light">
                    <a href="{{ route('login') }}"><span class="icon is-small"><i class="fas fa-sign-in-alt"></i></span> Inicia sesión</a> como adoptante para solicitar la adopción.
                </div>
            @endauth
        </div>
    </div>
